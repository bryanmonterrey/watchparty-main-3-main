...
export const CryptoPaymentDialog: React.FC<CryptoPaymentDialogProps> = ({ chargeId, amount }) => {
  ...
  useEffect(() => {
    if (chargeId) {
      fetch(`/api/get-crypto-charge-details?chargeId=${chargeId}`)
      ...
      {Object.keys(paymentOptions).length > 0 ? (
        <>
          ...
          {selectedNetwork && selectedCurrency && paymentOptions[selectedNetwork][selectedCurrency] && (
            <>
              <p className="mt-4">
                Please send {paymentOptions[selectedNetwork][selectedCurrency].amount} {paymentOptions[selectedNetwork][selectedCurrency].currency} 
        ...
        <p>No payment options available. Please try again.</p>
      )}
    </div>
  );
};